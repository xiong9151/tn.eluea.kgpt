...
public class SimpleAIController {
    ...
    public void generateResponse(String prompt, String systemMessage) {
        ...
        if (needModelClient() || needApiKey()) {
            String missingKeyMsg = "Missing API Key. Please configure your API key in KGPT settings.";
            try {
                android.content.Context ctx = tn.eluea.kgpt.ui.UiInteractor.getInstance().getContext();
                if (ctx != null) {
                    missingKeyMsg = ctx.getString(tn.eluea.kgpt.R.string.missing_api_key_generic);
                }
            } catch (Exception e) {
                // formatting or context error
            }
            publisher = new SimpleStringPublisher(missingKeyMsg);
        } else {
