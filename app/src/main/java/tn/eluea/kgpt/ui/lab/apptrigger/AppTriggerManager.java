...
public class AppTriggerManager {
    ...
    public AppTrigger findByTrigger(String triggerText) {
        ...
        String lowerTrigger = triggerText.toLowerCase(java.util.Locale.ROOT).trim();
        for (AppTrigger trigger : appTriggers) {
            if (trigger.isEnabled() && trigger.getTrigger().toLowerCase(java.util.Locale.ROOT).equals(lowerTrigger)) {
                return trigger;
            }
        }
        return null;
